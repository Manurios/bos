...
// menu all
const { listephoto, listtextpro } = require('./menu/creator.js')
...
const costum = (pesan, tipe, target, target2) => {
			frhan.sendMessage(from, pesan, tipe, { contextInfo: { forwardingScore: 508, isForwarded: true }, quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJid: "628311800241-1610483974@g.us" } : {}) }, message: { "videoMessage": { "url": "https://mmg.whatsapp.net/d/f/AlWkboTlPr0p16IFat9VcqYqHyG1iy_YdWVi7aWsvoT7.enc","fileLength": "40742","mediaKey": "c+ygokxZop+Q126P5OtJ3LgrTqtCBwe69KPDIwQJLuc=","caption": `${target2}`,"gifPlayback": true,"height": 272,"width": 224, "jpegThumbnail": imgbase64 } } } })
		}
...
if (isGroup) {
	...
	if(FXC7.includes('prefix')){
	  reply(`*_PREFIX JENNY BOT ADALAH [ ${prefix} ]_*`)
	}
	...
	break
		...
		case 'takestick':
			...
			if (isBanned) return reply(mess.only.benned)
				...
				costum(help(prefix, instagram, yt, name, pushname2, user, limitawal, uptime, JAM, tanggal(), groupName, chats, premi, groups, Simihh, Welcomee, Nsfww, BadWordd, AntiLinkk, AntiVirtexx, autostickernya, frhan, process, RAM, Ramadhan, Ucapan, premium), MessageType.text, FarhanGans, FarhanGans2)
			break  
		case 'daftar':
		case 'verify':
					frhan.updatePresence(from, Presence.composing)
		...
		case 'listsurah':
			...
			case 'infobot':
				...
				case 'stiker':
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						const media = await frhan.downloadAndSaveMediaMessage(encmedia, `./sticker/Frhan`)
						await ffmpeg(`${media}`)
								.input(media)
								.on('start', function (cmd) {
									console.log(`Started : ${cmd}`)
								...
								.on('end', function () {
									console.log('Finish')
									exec(`webpmux -set exif ./sticker/data.exif ./sticker/Frhan.webp -o ./sticker/Frhan.webp`, async (error) => {
										if (error) return reply(mess.error.bug)
										frhan.sendMessage(from, fs.readFileSync(`./sticker/Frhan.webp`), sticker, {quoted: mek})
										fs.unlinkSync(media)	
										fs.unlinkSync(`./sticker/Frhan.webp`)	
									})
								})
								.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
								.toFormat('webp')
								.save(`./sticker/Frhan.webp`)
					} else if ((isMedia && mek.message.videoMessage.fileLength < 10000000 || isQuotedVideo && mek.message.extendedTextMessage.contextInfo.quotedMessage.videoMessage.fileLength < 10000000)) {
					...
					exec(`webpmux -set exif ${addMetadata('M̴̀̈́̆̈́̽̕a̴̡̲̤͛͂ń̶̍̄urios', 'Punya Manu')} ${rano} -o ${rano}`, async (error) => {
...
//creator
				...
				if (args.length < 1) return reply('Harta Tahta Apa??')
					anu = body.slice(7)
					buffer = await getBuffer(`https://api.vhtear.com/hartatahta?text=${anu}&apikey=${VthearApi}`, {method: 'get'})
			...
			anu = await getBuffer(`https://mhankbarbar.moe/api/text2image?text=${ttptext}&apiKey=${BarBarApi}`, {method: 'get'})
					ttptxt = anu.result
					exec(`wget ${ttptxt} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
					fs.unlinkSync(ranp)
					if (err) return reply(mess.error.stick)
					exec(`webpmux -set exif ${addMetadata('M̴̀̈́̆̈́̽̕a̴̡̲̤͛͂ń̶̍̄urios', 'Punya Manu')} ${rano} -o ${rano}`, async (error) => {
					if (error) return reply(mess.error.stick)
				...
				if (args.length < 1) return reply('emoji??')
					...
					anu = await fetchFxc7(`https://mhankbarbar.moe/api/emoji2png?emo=${teks}&apiKey=${BarBarApi}`, {method: 'get'})
...
//akhir kreator
		    ...
		    try {
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 JENNY  〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 JENNY 〙', members_id, true)
					break
			    case 'kbbi':
			    try {
			    if (isBanned) return reply(mess.only.benned)    
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					if (mentioned.length > 1) {
						teks = 'Baiqlah, Mengeluarkan Karna Meresahkan :\n'
				...
				case 'kicktime':
					...
					setTimeout( () => {
					reply('Udah Sukses Kick')
					}, 7000)
					setTimeout( () => {
					frhan.groupRemove(from, mentioned)
					}, 6000)
					setTimeout( () => {
					frhan.sendMessage(from, `Say Good Bye Dulu @${mentioned[0].split('@')[0]}`,MessageType.text) // ur cods
					}, 5000)
					setTimeout( () => {
					frhan.sendMessage(from, 'Asikkk Dapet Makanan nihh:D',MessageType.text)
					}, 2500)
					setTimeout( () => {
					reply('Bismmillah Ngekick')
					}, 0)
					break
				case 'promote':
				if (isBanned) return reply(mess.only.benned)    
...
// akhir encrypt & decrypt Fitur
		...
		case 'tanggallahir':
			...
			if (!isUser) return reply(mess.only.userB)
			if (isLimit(sender)) return reply(ind.limitend(pusname))
			if (args.length < 1) return reply('tanggal berapa??\n_Example: !tanggallahir 01/05/2003')
			tggl = body.slice(14)
			ja = tggl.split("/")[0];
			di = tggl.split("/")[1];
			an = tggl.split("/")[2];
	...
	// new features
		...
		case 'restart':
		if (!isOwner) return reply(mess.only.ownerB)
		await costum('_*[ + ] RESTART SUCCESS*_', text, FarhanGans, FarhanGans2)
		return process.send('reset')
		break 
		case 'repeat':
			...
			costum(stdout, MessageType.text, FarhanGans, FarhanGans2)
				...
				frhan.sendMessage(from, util.format(e), MessageType.extendedText, {quoted: mek})
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[MANURIOS]','aqua'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
			}
		} catch (e) {
